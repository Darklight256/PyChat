...
##############################SET NEW PASSWORD
def set_new_password(name, fname, lname, hsh):
    sql = "SELECT MAX(uid) FROM `usr`"
    mycursor.execute(sql)
    UID = mycursor.fetchall()[0] + 1

    sql = "INSERT INTO `usr` (username, password, uid, fName, lName) VALUES (" + name + ", " + hsh + ", " + str(
        UID) + ", " + fname + ", " + lname + ")";
    mycursor.execute(sql)
    connection.commit()
...
def change_username (UID, new_name) :
    if valid_username(new_name) == False :
        sql = "UPDATE `usr` SET username = " + new_name + " WHERE uid = " + UID
        mycursor.execute(sql)
        connection.commit()
        return True
    return False
...
#############################CHANGE PASSWORD
def change_password(name, hsh, newhsh):
...
def human_test(g1=-1, g2=-1, g3=-1):
    hmn_ver_ques = ["Which of the following is an animal?", "Which one is prime?", "Which one is biggest?",
                    "Which one is smallest?", "What is the result of 1/2 + 1/4 + 1/8 + 1/16 + ... ?",
                    "What is the result of 1/3 + 1/9 + 1/27 + 1/81 + ... ?", "What is the biggest 5 digit number?"]
    hmn_ver_ans = [["1. Dog    2. Book    3. Chair    4. iPhone   5. Pasta",
                    "1. Kitchen    2. Airpod    3. Chicken    4. Door    5. CPU",
                    "1. Banana    2. Video    3. Google    4. Lion    5. Pen"],
                   ["1. 16    2. 13    3. 18    4. 4    5. 20", "1. 10    2. 555555577777    3. 98    4. 10000    5. 6",
                    "1. 11    2. 21    3. 24    4. 12    5. 90"],
                   ["1. 51    2. 1    3. -100    4. 51    5. 40", "1. 80    2. 21    3. 14    4. 82    5. 20",
                    "1. 0    2. -123    3. 1    4. -2    5. -85"],
                   ["1. 16    2. -13    3. 18    4. -4    5. 20", "1. 11    2. 587    3. 322    4. 12    5. 0",
                    "1. -1000    2. 1    3. 2    4. 3    5. 4"],
                   ["1. 3/4    2. 2/3    3. 1    4. 4/5    5. 2", "1. 2/5    2. 1    3. 2    4. 13/16    5. 7/8",
                    "1. 5/8    2. 33/34    3. 1/2    4. 1    5. 2"],
                   ["1. 1/2    2. 2/3    3. 1    4. 4/5    5. 2", "1. 5/8    2. 1/2    3. 8/9    4. 1    5. 2",
                    "1. 2/9    2. 1/81    3. 1/2    4. 1    5. 2/3"],
                   ["1. -99990    2. 99    3. 9999    4. 100000    5. none",
                    "1. 99    2. 99999    3. 999    4. 100000    5. none",
                    "1. 9234    2. -10000    3. 12345    4. 99999    5. 9090"]]
